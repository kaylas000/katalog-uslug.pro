...
| Сайт: каталог из API v2 | `config/site.json` → **`catalogApiBaseUrl`**; `npm run build:layout` добавляет `<meta name="katalog-catalog-api">`; `js/main.js` грузит **`GET /v1/catalog?v=2`** (пагинация `nextCursor`, сортировки), без `/data/catalog.json` |
